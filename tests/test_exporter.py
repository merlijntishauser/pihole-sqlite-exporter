...
class TestHealthReadyStatus:
    def test_health_status_ok(self, monkeypatch) -> None:
    ...
    def test_health_status_too_old(self, monkeypatch) -> None:
        monkeypatch.setattr(exporter.scraper.SETTINGS, "scrape_interval", 10)
        monkeypatch.setattr(
            exporter.metrics.METRICS,
            "get_snapshot",
    ...
    def test_health_status_failed_scrape(self, monkeypatch) -> None:
        monkeypatch.setattr(exporter.scraper.SETTINGS, "scrape_interval", 10)
        monkeypatch.setattr(
            exporter.metrics.METRICS,
            "get_snapshot",
    ...
    def test_ready_status_waiting(self, monkeypatch) -> None:
        monkeypatch.setattr(
            exporter.metrics.METRICS,
            "get_scrape_status",
            lambda: (0, 0.0, 0.0),
    ...
    def test_ready_status_ok(self, monkeypatch) -> None:
        monkeypatch.setattr(
            exporter.metrics.METRICS,
            "get_scrape_status",
